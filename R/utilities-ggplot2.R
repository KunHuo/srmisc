...
#' Set Global Parameters for Font Size and Family
#'
#' This function sets the global parameters for font size and font family by
#' updating the relevant options. It is useful for configuring the appearance
#' of plots and text output in a consistent manner across your R session.
#'
#' @param font.size Numeric value specifying the font size. If not provided,
#' the font size remains unchanged.
#' @param font.family Character string specifying the font family. If not
#' provided, the font family remains unchanged.
#'
#' @details
#' The function updates the options \code{sr.font.size} and \code{sr.font.family}
#' with the provided values. If a parameter is missing, its corresponding option
#' is not modified.
#'
#' @examples
...
#' global_params(font.family = 'Arial')
#'
#' # Set both the font size and family
#' global_params(font.size = 10, font.family = 'Times New Roman')
#'
#' @export
global_params <- function(font.size, font.family) {
  if(!missing(font.size)){
    options(sr.font.size = font.size)
  }

  if(!missing(font.family)){
    options(sr.font.family = font.family)
  }
}
...
#'
#' This function retrieves the global font size set in the options. If the global
#' font size is not set, it returns a default value of 9.
#'
#' @return Numeric value representing the global font size. If the global font
#' size is not set, the function returns 9.
#'
#' @details
#' The function checks the option \code{sr.font.size}. If this option is not set,
#' it returns a default font size of 9. Otherwise, it returns the value of the
#' \code{sr.font.size} option.
#'
#' @examples
#' # Get the current global font size
#' current_font_size <- global_fontsize()
#' print(current_font_size)
#'
#' @export
global_fontszie <- function(){
  res <- options("sr.font.size")
  if(is.null(res)){
    9
  }else{
    res
...
#'
#' This function retrieves the global font family set in the options. If the global
#' font family is not set, it returns a default value of "serif".
#'
#' @return Character string representing the global font family. If the global
#' font family is not set, the function returns "serif".
#'
#' @details
#' The function checks the option \code{sr.font.family}. If this option is not set,
#' it returns a default font family of "serif". Otherwise, it returns the value of the
#' \code{sr.font.family} option.
#'
#' @examples
#' # Get the current global font family
#' current_font_family <- global_fontfamily()
#' print(current_font_family)
#'
#' @export
global_fontfamily <- function(){
  res <- options("sr.font.family")
  if(is.null(res)){
    "serif"
  }else{
    res
